...
#remove professors and coal miners with extremely low IQs (there's likely to be a floor effect)
prof_dat = subset(prof_dat, IQ > 80)
coal_dat = subset(coal_dat, IQ > 65)
...
ggplot(total.data, aes(x = IQ, fill = Occupation)) + 
  ...
  annotate("text", x = 130, y = 38, label = " ← IQ of 106 (top 20% of machine operators)") +
